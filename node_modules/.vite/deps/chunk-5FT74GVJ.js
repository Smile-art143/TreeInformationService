...
function f(t3, e, s, n3, r, o2) {
  ...
  const i2 = [], u = new Pr();
  {
    const m2 = u.createGeometryUserIndex(), c2 = u.addGeometry(e), p = u.addGeometry(s), f2 = new Ka(o2);
    let C3 = 0, E2 = null;
    if (u.hasCurves()) {
      E2 = new _();
    ...
    u.dbgVerifyCurves(), f2.setEditShapeCrackAndCluster(u, n3), f2.cut(t3, m2, c2, p, i2);
    for (const t4 of i2) {
...
function C2(t3, e, s, n3, r, u) {
  ...
  const c2 = [], y = new Pr();
  {
    const t4 = y.createGeometryUserIndex(), p = y.addGeometry(e), f2 = y.addGeometry(s), C3 = new Ka(u);
    let E2 = 0, G2 = null;
    if (y.hasCurves()) {
      G2 = new _();
      ...
      aa(y, e2, n3.total(), 12e3, G2, null, u);
    }
    y.dbgVerifyCurves(), C3.setEditShapeCrackAndCluster(y, n3), C3.cut(false, t4, p, f2, c2), null !== G2 && G2.stitchCurves(y, cr, E2, true);
    let v = y.getGeometry(p);
    const w = new ur(), x2 = new ur(), I = [];
    for (let e2 = 0; e2 < c2.length; e2++) {
      let s2 = null;
      {
        const r2 = new Pr(), o2 = r2.addGeometry(v), d = r2.addGeometry(y.getGeometry(c2[e2]));
        if (r2.hasCurves()) {
        ...
        const p2 = C3.intersection(o2, d);
        if (null !== G2 && G2.stitchCurves(r2, cr, E2, true), s2 = r2.getGeometry(p2), s2.isEmpty()) continue;
        const _2 = y.getGeometryUserIndex(c2[e2], t4);
        2 === _2 ? w.add(s2, false) : 1 === _2 ? x2.add(s2, false) : I.push(s2);
      }
      {
        const t5 = new Pr(), s3 = t5.addGeometry(v), r2 = t5.addGeometry(y.getGeometry(c2[e2]));
        if (t5.hasCurves()) {
    ...
    if (!v.isEmpty() && c2.length > 0 && I.push(v), w.isEmpty() && x2.isEmpty()) return;
    if (!w.isEmpty()) {
      const t5 = g(w, 1);
      r.push(t5);
    }
...
function m(e, m2) {
  const c2 = C(e);
  return n2(x(e), P(m2), Z(c2)).map((r) => j(r, c2)).filter(N);
}
var c = o();
...
//# sourceMappingURL=chunk-5FT74GVJ.js.map

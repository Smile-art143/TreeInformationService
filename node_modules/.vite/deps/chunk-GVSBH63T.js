...
async function n3(n4, e3, p2) {
  const i3 = e3.toJSON();
  i3.contents = JSON.stringify(i3.contents), i3.token = await a2(e3.portalUrl, e3.apiKey, { signal: p2 == null ? void 0 : p2.signal, prompt: "no-prompt" !== (p2 == null ? void 0 : p2.authMode) });
...
var d = class {
  ...
  async translate(t2) {
    ...
    const e3 = (_a = this.portal.helperServices) == null ? void 0 : _a.aiUtilityServices;
    if (null == e3) return { success: false };
    const s5 = e3.url + e3.translateUtility;
    try {
      t2.requestSource ?? (t2.requestSource = "arcade");
...
function S(a3) {
  "async" === a3.mode && (a3.functions[r("TranslateText")] = function(t2, i3) {
    return a3.standardFunctionAsync(t2, i3, async (t3, a4, i4) => {
      ...
      const w2 = c.map((t4, e3) => new s4({ key: String(e3), text: t4 })), x2 = ((_a = t3.services) == null ? void 0 : _a.portal) ?? M.getDefault(), S2 = new i2({ to: u, contents: w2, from: g2, portalUrl: x2.restUrl.replace(/\/sharing\/rest$/, "") }), T2 = /* @__PURE__ */ new Map();
      let v = null;
      if (null != t3.lrucache) {
        const e3 = t3.lrucache;
        v ?? (v = y(S2)), S2.contents = (_b = S2.contents) == null ? void 0 : _b.filter((t4) => {
          const s5 = e3.getCachedTranslation(v, t4.text);
          return null == s5 || (T2.set(t4.key, { ...s5, key: t4.key, text: t4.text }), false);
        });
      }
      if ((_c = S2.contents) == null ? void 0 : _c.length) {
        const r2 = ((_d = t3.services) == null ? void 0 : _d.translation) ?? new d(x2, t3.console), n4 = await r2.translate(S2);
        if (!n4.success) return new p({ __proto__: null, success: false });
        for (const e3 of n4.results) {
          const r3 = (_f = (_e2 = S2.contents) == null ? void 0 : _e2.find((t4) => t4.key === e3.key)) == null ? void 0 : _f.text;
          if (null == r3) throw new n2(null, "NeverReach", null);
          T2.set(e3.key, e3), null != t3.lrucache && (v ?? (v = y(S2)), t3.lrucache.setCachedTranslation(v, r3, { detectedLanguage: e3.detectedLanguage, translation: e3.translation }));
        }
      }
...
//# sourceMappingURL=chunk-GVSBH63T.js.map

...
var $ = null;
var g = /^(?<hh>([01][0-9])|(2[0-3])):(?<mm>[0-5][0-9])(:(?<ss>[0-5][0-9]))?(\.(?<ms>\d+))?$/;
function j2(e2, t, n) {
  return e2.x < 0 ? e2.x += t : e2.x > n && (e2.x -= t), e2;
}
function x(e2, t, n, o) {
  const a = W(n) ? G(n) : null, u3 = a ? Math.round((a.valid[1] - a.valid[0]) / t.scale[0]) : null;
  return e2.map((e3) => {
...
function w(e2, n = 18, r2, o, i) {
  ...
  for (const { geometry: t, attributes: f2 } of e2) {
    const { x: e3, y: r3 } = t, c = Math.max(0, e3 - n), p = Math.max(0, r3 - n), d = Math.min(i, r3 + n), h = Math.min(o, e3 + n), y = +u3(f2);
    for (let t2 = p; t2 < d; t2++) for (let i2 = c; i2 < h; i2++) {
...
async function I(e2, t, n = true) {
  if (!t) return [];
  const { field: r2, field2: i, field3: s2, fieldDelimiter: l2, fieldInfos: u3, timeZone: f2 } = e2, c = r2 && (u3 == null ? void 0 : u3.find((e3) => e3.name.toLowerCase() === r2.toLowerCase())), m2 = !!c && xe(c), g2 = !!c && U(c), j3 = e2.valueExpression, x2 = e2.normalizationType, w2 = e2.normalizationField, I2 = e2.normalizationTotal, F2 = [], N2 = e2.viewInfoParams;
  let E = null, v = null;
  if (j3) {
    if (!$) {
    ...
    $.hasGeometryOperations(j3) && await $.enableGeometryOperations(), E = $.createFunction(j3), v = N2 ? $.getViewInfo({ viewingMode: N2.viewingMode, scale: N2.scale, spatialReference: new f(N2.spatialReference) }) : null;
  }
  const U3 = e2.fieldInfos, M2 = !(t[0] && "declaredClass" in t[0] && "esri.Graphic" === t[0].declaredClass) && U3 ? { fields: U3 } : null;
  return t.forEach((e3) => {
    const t2 = e3.attributes;
    let o;
    if (j3) {
      const t3 = M2 ? Object.assign({}, e3, { layer: M2 }) : e3, n2 = $.createExecContext(t3, v, f2);
      o = $.executeFunction(E, n2);
    } else t2 && (o = t2[r2], i ? (o = `${m(o)}${l2}${m(t2[i])}`, s2 && (o = `${o}${l2}${m(t2[s2])}`)) : "string" == typeof o && n && (g2 ? o = o ? new Date(o).getTime() : null : m2 && (o = o ? b(o) : null)));
...
function N(e2, t, n) {
  ...
  let i = "";
  return i = r2 && o ? U2(r2, o) : r2 || o, i ? "(" + i + ")" : "";
}
function U2(e2, t) {
  let n = null != e2 ? e2 : "";
...
//# sourceMappingURL=chunk-4OFHJCYT.js.map

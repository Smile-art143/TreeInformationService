...
function c(t2, o2) {
  const [n, e2, a2, l2] = m2(t2), s2 = a2 - n, f2 = l2 - e2, c2 = et, h2 = tt, u2 = Math.floor(0.5 * (c2 * o2 - h2)), y2 = (c2 - 2 * (u2 + h2)) / Math.max(s2, f2), d2 = Math.round(s2 * y2), x2 = Math.round(f2 * y2);
...
function h(t2, o2, n, a2, l2, s2, f2) {
  ...
  const g = et, p = tt, R = Math.floor(0.5 * (g * M - p)), w = (g - 2 * (R + p)) / x2, P = Math.round(y2 * w) + 2 * R, b = Math.round(d2 * w) + 2 * R;
  let C = 1;
  if (o2) {
...
function u(t2) {
  const o2 = l(t2.geom), n = s(o2), e2 = et, a2 = tt, f2 = Math.floor(0.5 * (e2 * t2.sdfPaddingRatio - a2)), m3 = e2 - 2 * (f2 + a2), c2 = m3 / Math.max(n.width, n.height), h2 = Math.round(n.width * c2) + 2 * f2, u2 = Math.round(n.height * c2) + 2 * f2, M = [];
...
//# sourceMappingURL=chunk-S2BKM32V.js.map

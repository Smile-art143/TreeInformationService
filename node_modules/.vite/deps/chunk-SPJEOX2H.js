...
var A = class _A {
  ...
  addPointEx(t2, s2) {
    ...
    if (o > 0) this.addPoint(t2);
      ...
      if (_A.buildPoint(i, this.m_distance, r - Wt, n2), this.m_offsetPts[this.m_offsetPtCount - 1] = n2.clone(), 1 === this.m_joins || 2 === this.m_joins) {
  ...
  buildOffset() {
    ...
    for (let o = 0; o < s2; o++) {
      ...
      let a2, f2, d = 0;
      {
        const t3 = r.x - h.x, s3 = r.y - h.y, e2 = m.x - h.x, _ = m.y - h.y;
        a2 = Math.atan2(s3, t3), f2 = Math.atan2(_, e2), this.m_a1 = a2, this.m_a2 = f2, 0 === o && (i = a2, n = f2), d = mi.orientationRobust(h.asPoint2D(), r.asPoint2D(), m.asPoint2D());
      }
      const u = f2;
      if (f2 < a2 && (f2 += Kt), !(Math.abs(Math.abs(a2 - f2) - Math.PI) < 1e-8)) if (d * this.m_distance > 0) if (1 === this.m_joins || 2 === this.m_joins) {
        ...
        if (1 !== this.m_joins) {
          ...
          if (2 === this.m_joins) {
            const s3 = r.x - h.x, e2 = r.y - h.y, i2 = m.x - h.x, n2 = m.y - h.y;
            let _ = (s3 * i2 + e2 * n2) / Math.sqrt(s3 * s3 + e2 * e2) / Math.sqrt(i2 * i2 + n2 * n2);
            if (_ = Math.max(_, -1), _ > 1 - 1e-8) {
            ...
            const P2 = Math.abs(this.m_distance / Math.sin(0.5 * Math.acos(_))), d2 = Math.abs(this.m_miterLimit * this.m_distance);
            if (P2 > d2) {
              ...
              const _2 = Math.sqrt(P2 * P2 - this.m_distance * this.m_distance), u3 = (P2 - d2) * Math.abs(this.m_distance) / _2;
              this.m_distance > 0 ? r2.rightPerpendicularThis() : r2.leftPerpendicularThis(), r2.scale(u3 / r2.length());
              const l3 = new mi();
              l3.setAdd(m2, r2);
              const y2 = new mi();
  ...
  handleClosedIntersection(t2, s2) {
    ...
    if (((i.x - e.x) * (o.y - n.y) - (i.y - e.y) * (o.x - n.x)) * this.m_distance < 0) {
      if (this.findIntersection(e, i, n, o, h) && !h.bAtExistingPt) {
        const r = 1e-8, m = Math.sqrt((i.x - e.x) * (i.x - e.x) + (i.y - e.y) * (i.y - e.y)), a2 = (i.x - e.x) / m, f2 = (i.y - e.y) / m, c = Math.sqrt((o.x - n.x) * (o.x - n.x) + (o.y - n.y) * (o.y - n.y)), _ = (o.x - n.x) / c, P2 = (o.y - n.y) / c;
        let d = false;
        const u = h.pt.clone();
        u.x += (a2 + _) * r, u.y += (f2 + P2) * r;
...
//# sourceMappingURL=chunk-SPJEOX2H.js.map

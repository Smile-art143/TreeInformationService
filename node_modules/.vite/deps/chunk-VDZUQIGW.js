...
function s(e, n, t2) {
  ...
  const l = (_a = o[n]) == null ? void 0 : _a.cache, i2 = l == null ? void 0 : l.get(t2);
  if (l && i2) {
    if (i2.refCount--, 0 === i2.refCount) {
      l.delete(t2);
      for (let e2 = 0; e2 < o.length; e2++) (_b = o[e2]) == null ? void 0 : _b.cache.delete(t2);
...
function m(e, n, t2) {
  ...
  if (null == n || null == o[n]) {
    for (let e2 = 0; e2 < o.length; e2++) {
      const n2 = (_a = o[e2]) == null ? void 0 : _a.cache.get(t2);
      if (n2) return n2.refCount++, n2.block;
    }
    return c.getBlock(e, t2);
  }
  const l = (_b = o[n]) == null ? void 0 : _b.cache.get(t2);
  if (l) return l.refCount++, l.block;
  for (let r2 = 0; r2 < o.length; r2++) {
    if (r2 === n || !o[r2]) continue;
    const e2 = (_c = o[r2]) == null ? void 0 : _c.cache, l2 = e2 == null ? void 0 : e2.get(t2);
...
function h(e, n, t2, o, l = null) {
  ...
  o.then(() => u2.isResolved = true).catch(() => u2.isRejected = true), (_a = i2[n]) == null ? void 0 : _a.cache.set(t2, u2);
}
function x(e, n, t2) {
  var _a;
  const o = r.get(e);
  o ? null != n && null != o[n] ? (_a = o[n]) == null ? void 0 : _a.cache.delete(t2) : c.deleteBlock(e, t2) : null == n && c.deleteBlock(e, t2);
}
function d(e, n) {
  const t2 = r.get(e);
...
function g(n, r2, c2, i2, u2, a2, f = null) {
  ...
  for (let d2 = 0; d2 < g2.length; d2++) {
    ...
    if (null == u2 && !(u2 = C(c3, p, n2, f))) return;
    const { pyramidLevel: s3, pyramidResolution: m3, excessiveReading: h3 } = fe(u2, x2, a2 || "closest");
    if (h3) return;
    const { storageInfo: M } = x2, { origin: R } = M, { x: C2, y: B } = m3, b = Math.max(0, Math.floor((r3.xmin - R.x) / C2)), j = Math.max(0, Math.floor((R.y - r3.ymax) / B)), v = Math.ceil(r3.width / C2 - 0.1), w = Math.ceil(r3.height / B - 0.1), $ = s3 > 0 ? M.pyramidBlockWidth : M.blockWidth, I = s3 > 0 ? M.pyramidBlockHeight : M.blockHeight, H2 = M.blockBoundary[s3];
    if (!H2) continue;
    const E = 1, P = Math.max(H2.minCol, Math.floor(b / $) - E), W = Math.max(H2.minRow, Math.floor(j / I) - E), z = Math.min(H2.maxCol, Math.floor((b + v - 1) / $) + E), F = Math.min(H2.maxRow, Math.floor((j + w - 1) / I) + E);
...
//# sourceMappingURL=chunk-VDZUQIGW.js.map

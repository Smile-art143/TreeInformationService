...
function expandedAncestors(el) {
  var _a;
  const ancestor = (_a = el.parentElement) == null ? void 0 : _a.closest(listItemSelector);
  if (!ancestor) {
    return;
  }
  ancestor.open = true;
...
function getListItemChildren(slotEl) {
  ...
  const groupChildren = assignedElements.filter((el) => el == null ? void 0 : el.matches(listItemGroupSelector)).map((group) => Array.from(group.querySelectorAll(listItemSelector))).flat();
  const listItemChildren = assignedElements.filter((el) => el == null ? void 0 : el.matches(listItemSelector));
  const listChildren = assignedElements.filter((el) => el == null ? void 0 : el.matches(listSelector));
  return {
    lists: listChildren,
...
@esri/calcite-components/dist/chunks/utils2.js:
  ...
  v3.3.3 *)
*/
//# sourceMappingURL=chunk-2CR7PJTV.js.map

...
var t = {};
t.defaultNoDataValue = S(-1 / 0), t.decode = function(s2, r2) {
  var o2 = (r2 = r2 || {}).encodedMaskData || null === r2.encodedMaskData, f2 = l(s2, r2.inputOffset || 0, o2), u = null != r2.noDataValue ? S(r2.noDataValue) : t.defaultNoDataValue, m = i(f2, r2.pixelType || Float32Array, r2.encodedMaskData, u, r2.returnMask), d = { width: f2.width, height: f2.height, pixelData: m.resultPixels, minValue: f2.pixels.minValue, maxValue: f2.pixels.maxValue, noDataValue: u };
...
var i = function(e, t2, i2, n2, a2) {
  ...
  for (var g, h, k = new Float32Array(m * d), x = 0; x <= u; x++) {
    ...
    if (0 !== p) for (var w = 0; w <= f2; w++) {
      var y = w !== f2 ? m : e.width % f2;
      if (0 !== y) {
        var V, B, v, U, D = x * e.width * d + w * m, M = e.width - y, b = e.pixels.blocks[o2];
        if (b.encoding < 2 ? (0 === b.encoding ? V = b.rawData : (s(b.stuffedData, b.bitsPerPixel, b.numValidPixels, b.offset, c, k, e.pixels.maxValue), V = k), B = 0) : v = 2 === b.encoding ? 0 : b.offset, i2) for (h = 0; h < p; h++) {
          for (7 & D && (U = i2[D >> 3], U <<= 7 & D), g = 0; g < y; g++) 7 & D || (U = i2[D >> 3]), 128 & U ? (r2 && (r2[D] = 1), l2[D++] = b.encoding < 2 ? V[B++] : v) : (r2 && (r2[D] = 0), l2[D++] = n2), U <<= 1;
          D += M;
        }
        else if (b.encoding < 2) for (h = 0; h < p; h++) {
          for (g = 0; g < y; g++) l2[D++] = V[B++];
          D += M;
        }
        else for (h = 0; h < p; h++) if (l2.fill) l2.fill(v, D, D + y), D += y + M;
        else {
          for (g = 0; g < y; g++) l2[D++] = v;
          D += M;
        }
...
var l = function(e, t2, i2) {
  var n2 = {}, a2 = new Uint8Array(e, t2, 10);
  if (n2.fileIdentifierString = String.fromCharCode.apply(null, a2), "CntZImage" != n2.fileIdentifierString.trim()) throw "Unexpected file identifier string: " + n2.fileIdentifierString;
  t2 += 10;
  ...
  if (n2.fileVersion = l2.getInt32(0, true), n2.imageType = l2.getInt32(4, true), n2.height = l2.getUint32(8, true), n2.width = l2.getUint32(12, true), n2.maxZError = l2.getFloat64(16, true), t2 += 24, !i2) {
    if (l2 = new DataView(e, t2, 16), n2.mask = {}, n2.mask.numBlocksY = l2.getUint32(0, true), n2.mask.numBlocksX = l2.getUint32(4, true), n2.mask.numBytes = l2.getUint32(8, true), n2.mask.maxValue = l2.getFloat32(12, true), t2 += 16, n2.mask.numBytes > 0) {
      ...
      do {
        if (r2 > 0) for (; r2--; ) s2[f2++] = l2.getUint8(o2++);
        else {
          var u = l2.getUint8(o2++);
          for (r2 = -r2; r2--; ) s2[f2++] = u;
        }
        r2 = l2.getInt16(o2, true), o2 += 2;
      } while (o2 < n2.mask.numBytes);
      if (-32768 !== r2 || f2 < s2.length) throw "Unexpected end of mask RLE encoding";
  ...
  for (var h = 1e9, k = 0, x = 0; x < g; x++) for (var p = 0; p < c; p++) {
    ...
    if (2 !== V.encoding) {
      if (0 !== B && 2 !== B) {
        if (B >>= 6, V.offsetType = B, 2 === B) V.offset = l2.getInt8(1), w++;
        else if (1 === B) V.offset = l2.getInt16(1, true), w += 2;
        else {
          if (0 !== B) throw "Invalid block offset type";
          V.offset = l2.getFloat32(1, true), w += 4;
        }
        if (h = Math.min(V.offset, h), 1 === V.encoding) if (B = l2.getUint8(w), w++, V.bitsPerPixel = 63 & B, B >>= 6, V.numValidPixelsType = B, 2 === B) V.numValidPixels = l2.getUint8(w), w++;
        else if (1 === B) V.numValidPixels = l2.getUint16(w, true), w += 2;
        else {
...
//# sourceMappingURL=LercWorker-XWRNPKAD.js.map
